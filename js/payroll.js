...
class PayrollManager {
  ...
  generatePayroll() {
    ...
    if (window.hrPortal && window.hrPortal.employees) {
      // Clear existing payroll for this month
  ...
  calculateBonus(baseSalary) {
    // Simple bonus calculation (5-15% of base salary)
    const bonusPercentage = Math.random() * 0.1 + 0.05; // 5-15%
    return Math.round(baseSalary * bonusPercentage);
  }

  calculateDeductions(baseSalary) {
    // Simple deductions calculation (taxes, social security, etc.)
